...
public class QueueDemo {
    ...
    public static void main(String[] args) throws NoSuchMethodException {
        ...
        System.out.printf(
                "Fixed Queue: %s%n",
                IntStream.range(0, 10).mapToObj(i -> fixed.get())
        ...
        System.out.printf(
                "Dynamic Queue: %s%n",
                IntStream.range(0, 10).mapToObj(i -> dynamic.get())
                         .map(CHARACTER_TOSTRING).collect(Collectors.joining()));
        ...
        System.out.printf(
                "Circular Queue (1st pass): %s%n",
                IntStream.range(0, 10).mapToObj(i -> circular.get())
                         .map(CHARACTER_TOSTRING).collect(Collectors.joining()));

        IntStream.range(10, 20).mapToObj(i -> INCREMENT_CHARACTER.apply('A', i)).forEach(circular::put);
        System.out.printf(
                "Circular Queue (2nd pass): %s%n",
                IntStream.range(0, 10).mapToObj(i -> circular.get())
                         .map(CHARACTER_TOSTRING).collect(Collectors.joining()));

        System.out.print("Store/Consume from circular queue: ");
        IntStream.range(0, 26).mapToObj(i -> {return circular.put(INCREMENT_CHARACTER.apply('A', i)).get().toString();}).forEach(System.out::print);

        dynamic.reset();
        System.out.printf("%nDynamic Queue (after reset):%s%n", dynamic.stream().map(CHARACTER_TOSTRING).collect(Collectors.joining()));
        
        Queue<Character> alphabet = new FixedQueue<Character>(26, 'a', INCREMENT_CHARACTER);
        copyQueueContents(alphabet, dynamic);
        System.out.printf("Dynamic Queue (after copy):%s%n", dynamic.stream().map(CHARACTER_TOSTRING).collect(Collectors.joining()));
    }
}
