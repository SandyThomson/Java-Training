...
public class Help {
	...
	public static boolean printOption(char option) {
		...
		case '1':
			Stream.concat(ifSyntax.stream(), exitStatement.stream()).forEach(
					System.out::println);
			break;
		case '2':
			Stream.concat(switchSyntax.stream(), exitStatement.stream())
					.forEach(System.out::println);
			break;
		case '3':
			Stream.concat(forSyntax.stream(), exitStatement.stream()).forEach(
					System.out::println);
			break;
		case '4':
			Stream.concat(whileSyntax.stream(), exitStatement.stream())
					.forEach(System.out::println);
			break;
		case '5':
			Stream.concat(doWhileSyntax.stream(), exitStatement.stream())
					.forEach(System.out::println);
			break;
		case '6':
			Stream.concat(breakSyntax.stream(), exitStatement.stream())
					.forEach(System.out::println);
			break;
		case '7':
			Stream.concat(continueSyntax.stream(), exitStatement.stream())
					.forEach(System.out::println);
			break;
		case 'q':
			return false;
